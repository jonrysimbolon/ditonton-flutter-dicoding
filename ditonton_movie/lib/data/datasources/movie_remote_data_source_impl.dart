import 'dart:convert';

import 'package:ditonton_core/common/api_config.dart';
import 'package:ditonton_core/common/server_exception.dart';
import 'package:ditonton_movie/data/datasources/movie_remote_data_source.dart';
import 'package:ditonton_movie/data/models/movie_detail_model.dart';
import 'package:ditonton_movie/data/models/movie_model.dart';
import 'package:ditonton_movie/data/models/movie_response.dart';

import 'package:http/http.dart' as http;

class MovieRemoteDataSourceImpl implements MovieRemoteDataSource {
  final http.Client client;

  MovieRemoteDataSourceImpl({required this.client});

  @override
  Future<List<MovieModel>> getNowPlayingMovies() async {
    final response = await client.get(
      Uri.parse('$tmdbBaseUrl/movie/now_playing?api_key=$tmdbApiKey'),
    );

    if (response.statusCode == 200) {
      return MovieResponse.fromJson(json.decode(response.body)).movieList;
    } else {
      throw ServerException();
    }
  }

  @override
  Future<MovieDetailResponse> getMovieDetail(int id) async {
    final response = await client.get(
      Uri.parse('$tmdbBaseUrl/movie/$id?api_key=$tmdbApiKey'),
    );

    if (response.statusCode == 200) {
      return MovieDetailResponse.fromJson(json.decode(response.body));
    } else {
      throw ServerException();
    }
  }

  @override
  Future<List<MovieModel>> getMovieRecommendations(int id) async {
    final response = await client.get(
      Uri.parse('$tmdbBaseUrl/movie/$id/recommendations?api_key=$tmdbApiKey'),
    );

    if (response.statusCode == 200) {
      return MovieResponse.fromJson(json.decode(response.body)).movieList;
    } else {
      throw ServerException();
    }
  }

  @override
  Future<List<MovieModel>> getPopularMovies() async {
    final response = await client.get(
      Uri.parse('$tmdbBaseUrl/movie/popular?api_key=$tmdbApiKey'),
    );

    if (response.statusCode == 200) {
      return MovieResponse.fromJson(json.decode(response.body)).movieList;
    } else {
      throw ServerException();
    }
  }

  @override
  Future<List<MovieModel>> getTopRatedMovies() async {
    final response = await client.get(
      Uri.parse('$tmdbBaseUrl/movie/top_rated?api_key=$tmdbApiKey'),
    );

    if (response.statusCode == 200) {
      return MovieResponse.fromJson(json.decode(response.body)).movieList;
    } else {
      throw ServerException();
    }
  }

  @override
  Future<List<MovieModel>> searchMovies(String query) async {
    final response = await client.get(
      Uri.parse('$tmdbBaseUrl/search/movie?api_key=$tmdbApiKey&query=$query'),
    );

    if (response.statusCode == 200) {
      return MovieResponse.fromJson(json.decode(response.body)).movieList;
    } else {
      throw ServerException();
    }
  }
}
